...
def BaggingOrder(data, save_path, config):
    start_time = time.time() # 시작시간을 기록한다
    
    info_path = os.path.join(save_path, "bagging_order_info.txt")
    ...
    fid.write("max_samples  : %f\n" % config.max_samples)
    fid.write("max_features : %f\n" % config.max_features) 
    ...
    bagging = BaggingClassifier(base_estimator=cart, max_samples=config.max_samples, max_features=config.max_features, n_estimators=config.epoch_num)
    
    bagging.fit(train_x, train_y)
    
    fid.write("classes      : %d\n" % bagging.n_classes_) 
    
    with codecs.open(save_path + '/bagging_order_train_model.txt', 'w', encoding='utf-8') as f:
        pickle.dump(bagging, f)
    
    
    output_config = OutputConfig()
    
    tr_matrix_scores = np.zeros([len(bagging.classes_), len(bagging.classes_)],dtype=float)
    tr_predictions = bagging.predict(train_x)
    ...
    va_matrix_scores = np.zeros([len(bagging.classes_), len(bagging.classes_)],dtype=float)
    va_predictions = bagging.predict(valid_x)
    ...
    h, m = divmod(m, 60)
    fid.write("Bagging order processing time : %d:%02d:%02f" % (h, m, s))
    fid.close()
