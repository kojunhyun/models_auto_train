...
def BaggingClick(data, save_path, config):
    start_time = time.time() # 시작시간을 기록한다
    
    info_path = os.path.join(save_path, "bagging_click_info.txt")
    ...
    with codecs.open(save_path + '/bagging_click_train_model.txt', 'w', encoding='utf-8') as f:
    ...
    h, m = divmod(m, 60)
    fid.write("Bagging click processing time : %d:%02d:%02f" % (h, m, s))
    fid.close()
