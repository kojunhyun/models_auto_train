...
class ModelConfig(object):
    ...
    def __init__(self):
        ...
        self.output_data_column_cnt = 2  # 결과데이터의 컬럼 개수
        
        self.hidden_dim = (100, 100)     # 각 셀의 (hidden)출력 크기
...
def MlpOrder(data, save_path, config):
    start_time = time.time() # 시작시간을 기록한다
    
    info_path = os.path.join(save_path, "mlp_order_info.txt")
    ...
    fid.write("hidden_dim          : (%d,%d)\n" % config.hidden_dim) 
    fid.write("epoch_num           : %d\n" % config.epoch_num) 
    fid.write("learning_rate       : %f\n" % config.learning_rate) 
    fid.write("optimization        : %s\n" % config.optimization)    
        
    train_x, train_y, valid_x, valid_y = remake_data(data, save_path)
    
    mlp = MLPClassifier(hidden_layer_sizes=config.hidden_dim, max_iter=config.epoch_num,
                       learning_rate_init=config.learning_rate, solver='adam')
    
    mlp.fit(train_x, train_y)

    fid.write("layers              : %d\n" % mlp.n_layers_) 
    fid.write("classes             : %d\n" % len(mlp.classes_)) 
    fid.write("best iteration      : %d\n" % mlp.n_iter_) 
    fid.write("best loss           : %f\n" % mlp.loss_) 
    fid.write("optimization        : %s\n" % config.optimization) 
    
    
    with codecs.open(save_path + '/mlp_order_train_model.txt', 'w', encoding='utf-8') as f:
        pickle.dump(mlp, f)

    
    output_config = OutputConfig()
    
    tr_matrix_scores = np.zeros([len(mlp.classes_), len(mlp.classes_)],dtype=float)
    tr_predictions = mlp.predict(train_x)
    ...
    va_matrix_scores = np.zeros([len(mlp.classes_), len(mlp.classes_)],dtype=float)
    va_predictions = mlp.predict(valid_x)
    ...
    output_config.best_iteration = mlp.n_iter_
    output_config.tr_loss = mlp.loss_
    ...
    h, m = divmod(m, 60)
    fid.write("MLP order processing time : %d:%02d:%02f" % (h, m, s))
    fid.close()
