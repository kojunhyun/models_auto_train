...
def remake_data(data, seq_length, save_path):
    ...
    if 1 in tmp.values():
        ...
        for c in tmp:
            if tmp[c] == 1:
                lower.append(c)
        order_min = np.array(lower).min()
        total_data[total_data[:,-1] > order_min] = order_min
    ######################################################
...
def LstmOrder(data, save_path, config):
    ...
    h, m = divmod(m, 60)
    fid.write("LSTM order processing time : %d:%02d:%02f" % (h, m, s))
    fid.close()
