...
class ModelConfig(object):
    ...
    def __init__(self):
        ...
        self.population_size = 10        # genetic programming population every generation   
        self.verbosity = 2                # print information and provide a progress bar
        
        self.pattern_size = 2
        
        self.epoch_num = 10              # 에폭 횟수(학습용전체데이터를 몇 회 반복해서 학습할 것인가 입력)
...
def TpotOrder(data, save_path, config):
    
    start_time = time.time() # 시작시간을 기록한다
    
    info_path = os.path.join(save_path, "tpot_order_info.txt")
    ...
    with codecs.open(save_path + '/tpot_order_train_model.txt', 'w', encoding='utf-8') as f:
    ...
    h, m = divmod(m, 60)
    fid.write("Tpot order processing time : %d:%02d:%02f" % (h, m, s))
    fid.close()
